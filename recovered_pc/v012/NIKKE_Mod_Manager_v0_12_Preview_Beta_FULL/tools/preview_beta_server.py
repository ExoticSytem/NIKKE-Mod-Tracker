...
def decrypt_nkab(raw: bytes) -> Tuple[bytes, str]:
    ...
    try:
        ...
        raise RuntimeError("Falta 'cryptography'. Ejecuta INSTALAR_Y_ABRIR.bat para instalar las dependencias.") from exc
...
def extract_spine(path: Path, outdir: Path) -> Dict[str, Any]:
    try:
        ...
        raise RuntimeError("Falta UnityPy. Ejecuta INSTALAR_Y_ABRIR.bat para instalar las dependencias.") from exc
