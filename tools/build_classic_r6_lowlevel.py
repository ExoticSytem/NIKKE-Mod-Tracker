...
base.PREVIEW_PY = base.PREVIEW_PY.replace('classic-r5-debugfix-001', 'classic-r6-lowlevel-002')
...
helpers = r'''
  ...
  function r6Script(src){
    return new Promise((resolve,reject)=>{
      ...
      const s=document.createElement('script'); s.src=src+'?v=r6lowlevel002'; s.async=false;
  ...
  function r6RuntimeInfo(){
    const g=window; const low=g.__NMM_SPINE_LOW__||{};
...
def copy_vendor_r6(root: Path) -> None:
    ...
    base.run(['npm', 'install', 'pixi.js@6.5.10', 'pixi-spine@3.1.0', '@pixi-spine/base@3.1.0', '@pixi-spine/runtime-3.7@3.1.0', '@pixi-spine/runtime-3.8@3.1.0', '@pixi-spine/runtime-4.1@3.1.0', 'esbuild@0.24.2', '--no-audit', '--no-fund'], ROOT)
    pixi_browser = ROOT / 'node_modules' / 'pixi.js' / 'dist' / 'browser' / 'pixi.min.js'
    if pixi_browser.exists(): shutil.copy2(pixi_browser, vendor / 'pixi.min.js')
    spine_bundle = ROOT / 'node_modules' / 'pixi-spine' / 'dist' / 'pixi-spine.umd.js'
    if spine_bundle.exists(): shutil.copy2(spine_bundle, vendor / 'pixi-spine.umd.js')
...
PIXI.spine = pixiSpine; PIXI.spine37 = spine37; PIXI.spine38 = spine38; PIXI.spine41 = spine41;
globalThis.PIXI = PIXI; globalThis.pixi_spine = pixiSpine;
globalThis.__NMM_SPINE_LOW__ = { version:'classic-r6-lowlevel-002', PIXI, pixiSpine, base, TextureAtlas, SpineClass, spine37, spine38, spine41, runtimes:[{name:'3.8',mod:spine38},{name:'3.7',mod:spine37},{name:'4.1',mod:spine41}], runtimeNames:['3.8','3.7','4.1'], pixiSpineKeys:Object.keys(pixiSpine||{}), baseKeys:Object.keys(base||{}) };
""", encoding='utf-8')
...
def write_zip_r6() -> None:
    ...
    for rel in ['app.py', 'README.md', 'PREVIEW_NOTES.md', 'INSTALAR_Y_ABRIR.bat', 'ABRIR.bat', 'web/app.js', 'web/index.html', 'src/preview.py']:
        ...
        if p.exists():
            ...
            s = s.replace('0.13 Classic R3 Spine', '0.13 Classic R6 Spine LowLevel').replace('0.13 Classic R4 Spine', '0.13 Classic R6 Spine LowLevel').replace('0.13 Classic R5 Spine Debug', '0.13 Classic R6 Spine LowLevel')
            s = s.replace('Classic R3', 'Classic R6').replace('Classic R4', 'Classic R6').replace('Classic R5', 'Classic R6')
            p.write_text(s, encoding='utf-8')
    if base.ZIP.exists(): base.ZIP.unlink()
    ...
    with zipfile.ZipFile(base.ZIP) as z:
        bad = z.testzip()
        if bad: raise RuntimeError(f'Zip corrupto en {bad}')
